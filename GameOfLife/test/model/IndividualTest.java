...
public class IndividualTest {
        ...
        for (int row = 0; row < matrix1.getMatrixHeight(); row++) {
            for (int col = 0; col < matrix1.getMatrixWidth(); col++) {
                ...
                if (randInt < 10) {
                    ...
                    matrix1.getIndividualMatrix()[row][col] = status;
                    Assert.assertEquals(matrix1.getIndividualMatrix()[row][col],status);
                }
            }
        }
    ...
    /**
     *
     * Method: deepCopy(Individual copyFrom)
     *
     */
    @Test
    public void testDeepCopy() throws Exception {
        ...
        Individual copyFrom = new Individual(temp);
        matrix1.deepCopy(copyFrom);
        for(int i=0;i<20;i++){
            for(int j=0;j<20;j++){
                Assert.assertTrue(matrix1.getIndividualMatrix()[i][j]==copyFrom.getIndividualMatrix()[i][j]);
